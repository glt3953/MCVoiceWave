...
#pragma mark - NXVoiceWaveView
@interface NXVoiceWaveView(){
...
//
@property (nonatomic, strong) NXVolumeQueue* volumeQueue;

@end
@implementation NXVoiceWaveView
...
{
    @autoreleasepool {
        [[NSThread currentThread] setName:@"com.ningxia.VoiceWave"];
...
-(void)showInParentView:(UIView *)parentView
{
    ...
    [self.firstLine.layer insertSublayer:self.firstShapeLayer atIndex:0];
    ...
    [self.secondLine.layer insertSublayer:self.secondShapeLayer atIndex:0];

    [self addSubview:self.fillLayerImage];
    _fillLayerImage.frame = self.bounds;
    [_fillLayerImage.layer insertSublayer:self.fillShapeLayer atIndex:0];
...
-(CAShapeLayer*)generateShaperLayerWithLineWidth:(CGFloat)lineWidth
{
    ...
    waveLine.strokeColor = _fillShapeColor.CGColor;
    waveLine.fillColor = [UIColor clearColor].CGColor;
//    waveLine.lineWidth = lineWidth;
    waveLine.lineWidth = 0;
...
{
    if (!_firstLine) {
        self.firstLine = [[UIImageView alloc] init];
...
-(CAShapeLayer*)fillShapeLayer
{
    if (!_fillShapeLayer) {
        ...
        _fillShapeLayer.fillColor = _fillShapeColor.CGColor;
...
-(NXVolumeQueue*)volumeQueue
{
    if (!_volumeQueue) {
        self.volumeQueue = [[NXVolumeQueue alloc] init];
    }
    return _volumeQueue;
}
@end
